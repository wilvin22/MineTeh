...
try {
    ...
    // Execute query
    $listings = $supabase->customQuery('listings', '*', implode('&', $filters) . '&order=' . $sort_by);
    ...
    foreach ($listings as &$listing) {
        $images = $supabase->customQuery('listing_images', '*', "listing_id=eq.{$listing['listing_id']}&order=image_id.asc&limit=1");
        
        if ($images && !empty($images)) {
            $listing['image'] = $images[0]['image_path'];
        } else {
            $listing['image'] = null;
        }
    }
